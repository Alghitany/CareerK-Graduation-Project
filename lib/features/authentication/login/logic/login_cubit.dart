...
class LoginCubit extends Cubit<LoginState> {
  ...
  void emitLoginStates(LoginRequestBody loginRequestBody) async {
    ...
    response.when(success: (loginResponse) async{
      if (isRememberMeChecked) {
        ...
        await SharedPrefHelper.setSecuredString(SharedPrefKeys.userRole, loginResponse.user?.role ?? '');
      } else {
  ...
  Future<void> saveUserAccessToken(String token) async {
    ...
    await SharedPrefHelper.setSecuredString(SharedPrefKeys.userAccessToken, token);
    DioFactory.setTokenIntoHeaderAfterLogin(token);
  }
  Future<void> saveUserRefreshToken(String token) async {
    // save refresh token to shared preferences
    await SharedPrefHelper.setSecuredString(SharedPrefKeys.userRefreshToken, token);
  }
